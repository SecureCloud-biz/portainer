...
func previousVersions(schedules []portainer.EdgeUpdateSchedule) map[portainer.EndpointID]string {

	slices.SortFunc(schedules, func(a portainer.EdgeUpdateSchedule, b portainer.EdgeUpdateSchedule) bool {
	...
	for _, schedule := range schedules {
		for environmentId, status := range schedule.Status {
			...
			if schedule.Type == portainer.EdgeUpdateScheduleRollback {
				props.skip = true
				props.skipReason = "has rollback"
				continue
			}

			if status.Status == portainer.EdgeUpdateScheduleStatusPending || status.Status == portainer.EdgeUpdateScheduleStatusError {
				props.skip = true
